...
public class Service {
    ...
    public int rowCount(Connection c) throws Exception {
        ...
        try {
            ...
            ResultSet rs = stmt.executeQuery(String.format("select count(*) from service")); 
    ...
    public Service[] listAll(Connection c) throws Exception{
        boolean isOpen=false;
        Service[] objs = new Service[rowCount(c)];
        ...
        try {
            ...
            int i=0;
            while(res.next()){
                objs[i] = new Service(res.getString("id"), res.getString("nom"));
                i++;
            }
            stat.close();
            res.close();
